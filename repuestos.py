...
def validar_respuesta(mensaje):
    ...
    seguir = True
    respuesta = input(
        f"{mensaje}, Presione ´S´ para continuar , ´N´ para no continuar: ")
    while continuar:
        if respuesta.upper() == "S":
            ...
            respuesta = input(
                f"La respuesta ingresa es invalida, Debe ingresar ´S´ para continuar , ´N´ para no continuar: ")
    return seguir
...
def lista_modelos_compatibles(modelos_compatibles):
    ...
    if not modelos_compatibles:
        while actualizar_mas:
            modelos_compatible = input(
                "Ingrese un modelo compatible con el repuesto ingresado: ")
            modelos_compatibles.append(modelos_compatible)
            actualizar_mas = validar_respuesta(
                "Quiere ingresar mas modelos compatibles ?")
    else:
        while actualizar_mas:
            ...
            if opcion == 1:
                ...
                modelo_seleccionado = validar_opciones(imprimir_menu_modificar_modelo, 0, len(
                    modelos_compatibles), modelos_compatibles)
                del modelos_compatibles[int(modelo_seleccionado)]
            actualizar_mas = validar_respuesta(
                "Quiere modifcar algun modelo compatibles ?")
...
def ingresar_accesorio(coleccion_repuestos):
    ...
    repuesto = {datos_repuesto[0]: precio, datos_repuesto[1]: sku, datos_repuesto[2]
        : nombre, datos_repuesto[3]: marca, datos_repuesto[4]: compatible}
...
def modificar_accesorios(coleccion_respuestos):
    ...
    if opcion == 0:
        ...
        acualizacion = {"$set": {datos_repuesto[2]: input(
            "Ingresar el nombre del repuesto: ")}}
    elif opcion == 3:
        acualizacion = {"$set": {datos_repuesto[3]: input(
            "Ingresar la marca del repuesto: ")}}
    elif opcion == 4:
        repuesto = coleccion_respuestos.find_one(filtro)
        acualizacion = {"$set": {datos_repuesto[4]: lista_modelos_compatibles(
            repuesto.get(datos_repuesto[4], []))}}
    if opcion != 5:
...
def eliminar_accesorio(coleccion_repuestos):
    ...
    respuesta = validar_respuesta(
        f"Desea eliminar el repuesto con el siguiente ´SKU´: {sku},")
    if respuesta:
        filtro = {datos_repuesto[1]: sku}
...
def consultar_accesorios(coleccion_repuestos):
    ...
    print(f"\n{datos_repuesto[0]}{valor.rjust(10)} {datos_repuesto[1]}{valor.rjust(12)} {datos_repuesto[2]}{
          valor.rjust(14)} {datos_repuesto[3]} {valor.rjust(10)} {datos_repuesto[4]} {valor.rjust(10)}")
    for dato in repuestos:
        compatible_str = ', '.join(dato[datos_repuesto[4]]) if isinstance(
            dato[datos_repuesto[4]], list) else dato[datos_repuesto[4]]
        print(f"{str(dato[datos_repuesto[0]]).ljust(10)}{(dato[datos_repuesto[1]]).center(20)}{
              (dato[datos_repuesto[2]]).center(12)}{(dato[datos_repuesto[3]]).rjust(16)} {compatible_str.center(32)}")
...
def consultar_accesorio_especifico(coleccion_respuestos):
    ...
    print(f"{datos_repuesto[0]}{valor.rjust(10)} {datos_repuesto[1]}{valor.rjust(12)} {datos_repuesto[2]}{
          valor.rjust(14)} {datos_repuesto[3]} {valor.rjust(10)} {datos_repuesto[4]}{valor.rjust(10)} ")
    compatible_str = ', '.join(repuesto[datos_repuesto[4]]) if isinstance(
        repuesto[datos_repuesto[4]], list) else repuesto[datos_repuesto[4]]
    print(f"{str(repuesto[datos_repuesto[0]]).ljust(10)}{(repuesto[datos_repuesto[1]]).center(20)}{
          (repuesto[datos_repuesto[2]]).center(12)}{(repuesto[datos_repuesto[3]]).rjust(16)} {compatible_str.center(32)}")
...
def validar_opciones(menu_imprimir, numero_menor, numero_mayor, *args):
    ...
    while not opcion_valida:
        ...
        if numero_menor - 1 < opcion < numero_mayor + 1:
            ...
            print(f"La opcion ingresada no es valida, tiene que ser entre {
                  numero_menor} y {numero_mayor}")
    return opcion
...
def conectar_base_datos(mongo_url, base_datos, coleccion):
    try:
...
def main():
    mongo_url = conexion
    coleccion_repuestos = conectar_base_datos(mongo_url, base_datos, coleccion)
    if coleccion_repuestos is not None:
        continuar = True
        while continuar:
            opcion = validar_opciones(imprimir_menu, 1, 6)
            cant_repuestos = coleccion_repuestos.count_documents({})
            if opcion == 1:
                ingresar_accesorio(coleccion_repuestos)
            elif coleccion_repuestos.count_documents({}) > 0 and opcion != 6:
                if opcion == 2:
                    modificar_accesorios(coleccion_repuestos)
                elif opcion == 3:
                    eliminar_accesorio(coleccion_repuestos)
                elif opcion == 4:
                    consultar_accesorios(coleccion_repuestos)
                elif opcion == 5:
                    consultar_accesorio_especifico(coleccion_repuestos)
            elif coleccion_repuestos.count_documents({}) == 0 and opcion != 6:
